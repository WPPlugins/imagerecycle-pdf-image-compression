        ...
        <?php             
         ...
         if( !empty($this->settings['wpio_api_key']) &&  !empty($this->settings['wpio_api_secret']) ) {
            ...
            if($return && (floatval($return->quota_allowed)>0) ) {           
                ...
                $lasterr = $ioa->getLastError();
                $this->db_log( $lasterr, 0);
            }
            ...
            <div style="float: left;width:100%;clear:both">
                ...
                <div class="wpio-progress-wrap" style="float: left">
                    <div class="wpio-progress-bar" style="width: <?php echo $percentQuota;?>%;min-width:0.5%"></div>
                    <span><?php echo $percentQuota;?>%</span>
                </div>    
                
                <div style="float: left;margin:10px 10px 10px 20px">
                <a href="https://www.imagerecycle.com/prices" target="_blank" class="button button-primary"><?php _e('Get optimization quota','wpio') ?></a>
                </div>             
            </div>    
...
<!-- Display 10 latest errors -->
            ...
            <div class="accordion-section control-section control-section-default">
                <h3 class="accordion-section-title"  tabindex="0"><?php _e('ImageRecycle log', 'wpio') ?></h3>
                <ul class="accordion-section-content">
                    <li class="customize-control log_heading" style="display: list-item;" >
                        <span class="errCode"><?php _e('Error code', 'wpio') ; ?></span>
                        <span class="errTime"><?php _e('Logged time', 'wpio'); ?></span>
                        <span class="errMsg"><?php _e('Error Message', 'wpio'); ?></span>
                    </li>
                    <?php
                    if (!empty($wpio_logs) ):
                        foreach ($wpio_logs as $wpio_log):
                            ?>
                            <li class="customize-control" style="display: list-item;" >
                                <span class="errCode"><?php echo $wpio_log['code']; ?></span>
                                <span class="errTime"><?php echo  date( $display_format, $wpio_log['time'] ); ?></span>
                                <span class="errMsg"><?php echo $wpio_log['msg']; ?></span>
                            </li>
                            <?php
                        endforeach;
                    endif;
                    ?>
                    <li>
                        <p class="description"><?php _e('10 latest errors encountered', 'wpio'); ?></p>
                    </li>
                </ul>
            ...
            <div style="float: left;width:100%; border-top:1px solid #333; margin:20px 0 0 0;padding-top:10px">
                <div style="float: left;">
                    <p class="description" style="color: #f00">
                        <a id="wpio_reinitialize" class="button button-secondary"><?php _e('Reinitialize','wpio') ?></a>
    ...
    public function showImageResize(){
        ...
        if($api_imageresize == 1){
            echo '<label><input id="wpio_api_resize_auto_yes" name="_wpio_settings[wpio_api_resize_auto]" type="radio" value="1" checked>'. __('Yes','wpio') .'</label>'.
                    ...
                    '<br><label><input type="checkbox" id="wpio_api_resize_date_notmatter_val" name="_wpio_settings[wpio_api_resize_date_notmatter]" value="1" '.$checked. '/>'. __('Resize also images present before plugin installation', 'wipo') .'</label>';
        }else{
            echo '<label><input id="wpio_api_resize_auto_yes" name="_wpio_settings[wpio_api_resize_auto]" type="radio" value="1">'. __('Yes','wpio') .'</label>'.
                    ...
                    '<br><label style="margin-left:15px"><input type="checkbox" id="wpio_api_resize_status_val" name="_wpio_settings[wpio_api_resize_date_notmatter]" value="1" '.$checked. ' disabled/>'. __('Resize also images present before plugin installation', 'wipo') .'</label>';
        }
    }
