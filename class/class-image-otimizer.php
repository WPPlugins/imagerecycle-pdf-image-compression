...
class wpImageRecycle {
	...
	add_action('wp_ajax_wpio_revert', array(&$this,'doActionRevert'));
        ...
        if( !function_exists('curl_version')) {             
            $wpio_curl_notice_disabled  = get_option( 'wpio_curl_notice_dismissed', 0);   
            
            if(empty($wpio_curl_notice_disabled) ) {  
                add_action('admin_print_footer_scripts', array(&$this,'wpio_notice_dismissed_js'));  
                add_action( 'admin_notices', array($this, 'wpio_curl_notice') );
                add_action('wp_ajax_wpio_dismiss_curl_notice', array($this, 'dismissCurlNotice') );
            }
        }               
    ...
    public static function update_db_check() {
        ...
        if ( get_site_option( 'wpio_db_version' ) != WPIO_IMAGERECYCLE_VERSION ) {
            if( get_site_option( 'wpio_db_version' ) === false) {
    ...
    public static function install(){
	    global $wpdb;
       // if( !function_exists('curl_version')) {            
           // die('<p>The <strong>ImageRecycle pdf & image compression</strong> plugin requires curl installed in your PHP.</p>');
        //}
    ...
    function wpio_heartbeat_received( $response, $data ) {
        ...
        if( isset($data['wpio_heartbeat']) && $data['wpio_heartbeat'] == 'queue_process' ) {
            $this->headRequest(WPIO_IMAGERECYCLE_URL.'/queue_process.php');       
    ...
    public function headRequest($url) {
        if( function_exists('curl_version')) {
			...
			if($errno = curl_errno($ch)) {
				...
				$this->write_log("cURL error (".$errno."):\n". $error_message );
			}

            curl_close ($ch);

			//if curl error try other method
			if($errno) {

				$url = str_replace("https","http", $url); //fix ssl error
    ...
    function dismissCurlNotice() {              
        update_option( 'wpio_curl_notice_dismissed', 1);        
        $this->ajaxReponse(true);
    }
    
    function wpio_curl_notice() {
    ?>
        <div class="notice error wpio_curl_notice is-dismissible" >
            <p><?php _e( 'ImageRecycle has detected that curl php extension is not installed, the plugin will use a fallback function but it may not work on some servers. Please install curl if you can for a better experience', 'wpio' ); ?></p>
        </div>
    ...
    ?>
        ...
        (function($){                       
            ...
            $(document).on( 'click', '.wpio_curl_notice .notice-dismiss', function() {
                $.ajax({
                    url: ajaxurl,
                    data: {
                        action: 'wpio_dismiss_curl_notice'
                    }
                })
            })
...
echo '</div>';  //close div for wrap
	...
	//}
        ...
        if($resize_auto && $resize_maxsize) {   //Only apply on new images
            ...
            if($size && ($size[0]> $resize_maxsize)) {
                ...
                elseif ($does_date_matter){
                    $fparams['resize'] +=  array("width"=> $resize_maxsize);
                }
            }            
            if($size && ($size[1]> $resize_maxheight)) {
                if ($fileCreated > $installed_time) {
	...
	$return = $ioa->uploadFile($file,$fparams); 
        //$return = $ioa->uploadUrl($file,$fparams);
	if($return === false || $return === null || is_string($return) ){
	    $response->msg = $ioa->getLastError();
            ...
            $this->db_log($response->msg,$response->errCode);
            return $response;
	}else {
            //reset fail counter
