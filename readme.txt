...
**ImageRecycle is an automatic Image and PDF content optimizer for WordPress website.** Your customers are waiting for your pages to load, it can takes long, especially on mobile devices. Even content is served in seconds by a fast server, slow connection can make your users leaving.
...
More case studies available here: https://www.imagerecycle.com/case-studies

Plus, our WordPress plugin is fully using the WordPress framework to be as easy and fast as possible.

### We reduce image and pdf, not quality

The jpg, png, gif optimization algorithm is capable of reducing the image size by up to 85% (5x +), while the resulting photos are visually identical to the originals. We believe that image and pdf compression is a matter preserving the quality then reduce the size. 
...
The plugin is highly configurable and you can choose media from any folder on your server.
So it will work with all third party plugins including WooCommerce. Plus, you can of course optimize media from other folders (multiple site, other CMS...)
...
**More details here, Image Recycle:** https://www.imagerecycle.com/cms/wordpress
...
Our server is  a dedicated high speed server that host the optimization algorythm.
Like some others plugin we don't do any optimization process on YOUR server, it would be either not very effiscient or it would slow down a lot your server.
...
Globally use Best saving for a final usage of your image and Original quality if you've planned to make treatment later on images (with Photoshop for example).
...
The API key and secret is like a login that control that you are allowed to use this server.
...
= Will ImageRecycle optimization work if my website is using a CDN (like CloudFare or Maxxcdn)? =
...
= 3.1.0 : =
* Adding width and height options to resize images
* Adding option to resize or not images present before installation

= 3.0.1 : =
* Fix updater script

= 3.0.0 : =
* Index all media manager images for a faster image listing and optimization
* Add EXIF selection remover on optimization from API
* Add a progress bar with an image count when running the optimization
* Add an ImageRecycle error log accessible from the configuration
* Add md5 sum to be able to detect if an image has been replaced
* Possibility to reinitialize image indexation in order to recompress everything
* Add a new progrees bar design in image listing
* Change the button status (enable/disable) to a checkbox: "Activate media optimization"

= 2.1.1 : =
* Optimization to speed up listing files

= 2.1.0 : =
* Add WordPress standard language files
* Add dashboard widget to display optimization image stats

= 2.0.3 : =
* Content multi select don't work after ordering images with column titles

= 2.0.2 : =
* Fix new images not optimized on some configurations

= 2.0.1 : =
* Use curl instead of file_get_contents when possible

= 2.0.0 : =
* Launch full media optimization process in background
* Email notification when notification is done
* New image and PDF filtering system by: All media / Optimized / Non Optimized
* AJAX media ordering in admin: Filename, Size, Status after optimization, Status before optimization
* Add a search engine for media
* Add a bulk action to revert optimized images and PDF to original
* Display the quota consumption of optimized content works with both main account and sub accounts 
* Switch to new API version, drastic image optimization speed improvement (+50%)

= 1.1.2 : =
* Fix some default settings for optimized media and plugin
...
* Enhanced returned error messages from API (if optimization error occurs)
...
= 1.0.3 : =
* Add folder tree to include folders from optimization

= 1.0.2 : =
* Add text presentation of ImageRecycle, image and PDF optimization

= 1.0.1 : =
* Fix bulk optimization stuck on some servers (siteground)

= 1.0.0 : =
* Initial release of ImageRecycle, image and PDF optimization
